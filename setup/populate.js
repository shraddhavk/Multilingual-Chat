...
async function scriptStart(publishKey, subscribeKey) {
    ...
    const numberOfSpaces = data.spaces.length;
                        ...
                        pubnub.createSpace(
                            ...
                            status => {
                                if (!status.error) {
                                    ...
                                    if (status.statusCode === 409) {
                                        //skip duplicate spaces ie that already exists
                                        resolve();
                                    } else {
                                        console.log(
                                            `\ncreateSpace ${item.id} error:`,
                                            status.errorData ? status.errorData : status.message,
                                        );
                                        reject();
                                    }
                                }
                ...
                new Promise((resolve, reject) => {
                    setTimeout(() => {
                        ...
                        if (data.members[index].members.length > membersPerRequest) {
                            ...
                            const devideMembersArray = [];
                            while (leftMembersToAdd > 0) {
                                let sliceMembersStart =
                                    leftMembersToAdd - membersPerRequest > 0 ? leftMembersToAdd - membersPerRequest : 0;
                                devideMembersArray.push(
                                    data.members[index].members.slice(sliceMembersStart, leftMembersToAdd),
                                );
                                leftMembersToAdd = leftMembersToAdd - membersPerRequest;
                            }
                            for (const members of devideMembersArray) {
    ...
    if (createdUsers.length > 0 && createdSpaces.length > 0) {
        //Do not make memberships calls if there are no users or spaces
        createMemberships();
        values = await Promise.all(
            createdMemberships.map(p =>
                p.then(
                    () => ({ status: 'fulfilled' }),
                    () => ({ status: 'rejected' }),
                ),
